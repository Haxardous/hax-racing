...
--	local r, g, b = getColorFromString(string.upper(get("color")))
	if getElementType(toElement) == 'console' then
		...
		outputChatBox("[RACE] #FFFFFF".. message .. "#389583.", toElement, 56 ,149 ,131, true)
	end
end
...
addCommandHandler('redo',
	function( player, command, value )
		if isPlayerInACLGroup(player, g_GameOptions.admingroup) then
			local currentMap = exports.mapmanager:getRunningGamemodeMap()
			if currentMap then
				outputChatBox('#389583[RACE] #FFFFFFMap restarted by #389583' .. getPlayerName(player) .. '#FFFFFF.', g_Root, 255, 255,255 , true)
	...
	function( player, command, value )
		if isPlayerInACLGroup(player, g_GameOptions.admingroup) then
			if not stateAllowsRandomMapVote() or g_CurrentRaceMode:getTimeRemaining() < 1000 then
				...
				outputChatBox('#389583[RACE] #FFFFFFCurrent map ' .. choice[math.random( 1, #choice )] .. ' by #389583' .. getPlayerName(player) .."#FFFFFF.", g_Root, 0, 0, 0, true)
				startRandomMap()
			end
		end
	end
...
addCommandHandler('nextmap',
	function( player, command, ... )
		...
		g_ForcedNextMap = map
		outputChatBox('#389583[RACE] #FFFFFFNext map set to #389583' .. getMapName( g_ForcedNextMap ) .. ' #FFFFFFby #389583' .. getPlayerName( player ).."#FFFFFF.", g_Root, 255, 255, 255, true)
	end
)
