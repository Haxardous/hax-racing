...
addEventHandler('onClientResourceStart', g_ResRoot,
	function()
		...
		--fonta_arialbold = dxCreateFont("fonts/arialbold.ttf", 9)
		...
		g_dxGUI = {
			...
			ranksuffix = dxText:create('', screenWidth - 40, screenHeight - 86, false, "bankgothic", 0.8),
			checkpoint = dxText:create('', screenWidth - 15, screenHeight - 54, false, "bankgothic", 0.8, 'right'),
			mapdisplay = dxText:create('', 2, screenHeight - dxGetFontHeight(0.7, "bankgothic")/2, false, "bankgothic", 0.7, 'left')
		}
		...
		g_GUI = {	
			...
			roundedimg        = guiCreateStaticImage(screenWidth - 140, screenHeight - 153, 131, 127, ":race/img/roundedimg.png", false), ---1779, 925, 131, 127
			positonlabel      = guiCreateLabel(screenWidth - 138, screenHeight - 126, 119, 47, "0st", false, roundedimg), -- positonlabel (+3) for checkpointlabel
			checkpointlabel   = guiCreateLabel(screenWidth - 130, screenHeight - 80, 113, 30, "0/0", false, roundedimg),

			timeleftbg        = guiCreateStaticImage(screenWidth/2-108/2, 10, 168, 24, "img/countershape.png", false, nil),
			clockimg          = guiCreateStaticImage(screenWidth/2-0+45/2, 14, 16, 16, ":race/img/clockicon.png", false, mapinfobackground),
			timeleft          = guiCreateLabel(screenWidth/2-95/2, 10, 58, 24, "", false, mapinfobackground),
			thetimepassed     = guiCreateLabel(screenWidth/2-(-95)/2, 10, 58, 24, "0:00:00", false, mapinfobackground),

			healthbar         = FancyProgress.create(250, 1000, 'img/progress_health_bg.png', -17, screenHeight - 190, 123, 25, 'img/progress_health.png', 7, 5, 109, 15),
			speedbar          = FancyProgress.create(0, 1.5, 'img/progress_speed_bg.png', -65, 90, 123, 30, 'img/progress_speed.png', 8, 8, 108, 15),
		}

		timelefttimepassedfont 	= guiCreateFont("fonts/arialbold.ttf", 11)
		positonlabelfont       = guiCreateFont("fonts/arialbold.ttf", 39)
		checkpointlabelfont     = guiCreateFont("fonts/arialbold.ttf", 19)

		guiSetAlpha(g_GUI.roundedimg, 0.60)
		guiSetFont(g_GUI.positonlabel, positonlabelfont)
		guiLabelSetHorizontalAlign(g_GUI.positonlabel, "center", false)
        guiLabelSetVerticalAlign(g_GUI.positonlabel, "center")

        guiSetFont(g_GUI.checkpointlabel, checkpointlabelfont)
        guiLabelSetHorizontalAlign(g_GUI.checkpointlabel, "center", false)
        guiLabelSetVerticalAlign(g_GUI.checkpointlabel, "center")  

		guiSetFont(g_GUI.thetimepassed, timelefttimepassedfont)
        guiLabelSetHorizontalAlign(g_GUI.thetimepassed, "center", false)
		guiLabelSetVerticalAlign(g_GUI.thetimepassed, "center") 
		
        guiSetFont(g_GUI.timeleft, timelefttimepassedfont)
        guiLabelSetHorizontalAlign(g_GUI.timeleft, "center", false)
        guiLabelSetVerticalAlign(g_GUI.timeleft, "center") 
		g_GUI.speedbar:setProgress(0)

		hideGUIComponents('timeleftbg', 'timeleft', 'healthbar', 'speedbar', 'ranknum', 'ranksuffix', 'checkpoint', 'thetimepassed', 'clockimg', 'roundedimg', 'positonlabel', 'checkpointlabel')
...
function initRace(vehicle, checkpoints, objects, pickups, mapoptions, ranked, duration, gameoptions, mapinfo, playerInfo)
	local screenWidth, screenHeight = guiGetScreenSize()
	local sxlol, sylol = guiGetScreenSize()
	...
	--g_dxGUI.mapdisplay:text("Map: "..g_MapInfo.name)
		...
		setFarClipDistance( 1500 ) -- map load distance
		setFogDistance( 700 ) -- fog distance
		outputConsole("setFarClipDistance = 1500 / setFogDistance = 700")

	mapinfobackground = guiCreateStaticImage(sxlol*( 10/1024 ), screenHeight - 50, 367, 39, "img/shape.png", false)
	GUIEditor.staticimage[2] = guiCreateStaticImage(19, 6, 19, 27, "img/icon.png", false, mapinfobackground)
	mapinfolabel = guiCreateLabel(50, 6, 307, 27, g_MapInfo.name, false, mapinfobackground)
	guiSetAlpha(mapinfolabel, 0.75)
	guiSetAlpha(mapinfobackground, 0.8)
	local font0_arialbold = guiCreateFont("fonts/arialbold.ttf", 14)
	guiSetFont(mapinfolabel, font0_arialbold)
	guiLabelSetVerticalAlign(mapinfolabel, "center")  
	...
	-- GUI
	guiSetText(g_GUI.thetimepassed, "0:00:00")
	--g_dxGUI.timepassed:text('0:00:00')
	showGUIComponents('healthbar', 'speedbar')
	hideGUIComponents('timeleftbg', 'timeleft', 'clockimg', 'thetimepassed', 'positonlabel')
	if ranked then
		showGUIComponents('ranknum', 'ranksuffix', 'roundedimg', 'positonlabel', 'checkpointlabel')
	else
		hideGUIComponents('ranknum', 'ranksuffix', 'roundedimg', 'positonlabel', 'checkpointlabel')
	end
	if #g_Checkpoints > 0 then
		showGUIComponents('checkpointlabel')
	else
		hideGUIComponents('checkpointlabel')
	end
...
function launchRace(duration)
	...
	if type(duration) == 'number' then
		showGUIComponents('timeleftbg', 'timeleft', 'clockimg', 'thetimepassed')
...
function updateTime()
	...
	if not isPlayerFinished(g_Me) then
		guiSetText(g_GUI.thetimepassed, msToTimeStr(msPassed))
...
function setRankDisplay( rank )
	theranksuffex = (getElementData(g_Me, 'race rank') < 10 or getElementData(g_Me, 'race rank') > 20) and ({ [1] = 'st', [2] = 'nd', [3] = 'rd' })[getElementData(g_Me, 'race rank') % 10] or 'th';
	if not tonumber(rank) then
		guiSetText(g_GUI.positonlabel, '')
		return
	end
	guiSetText(g_GUI.positonlabel, getElementData(g_Me, 'race rank').. '' ..theranksuffex)
...
function showNextCheckpoint(bOtherPlayer)
	...
	guiSetText(g_GUI.checkpointlabel, (i - 1) .. ' / ' .. #g_Checkpoints)
	if i > 1 then
...
function checkpointReached(elem)
	...
	if g_CurrentCheckpoint < #g_Checkpoints then
		...
		guiSetText(checkpointlabel, #g_Checkpoints .. ' / ' .. #g_Checkpoints)
...
function Spectate._start()
	...
	g_GUI.speclabel = guiCreateLabel(screenWidth/2 - 100, screenHeight - 110, 200, 70, '', false)
	Spectate.updateGuiFadedOut()
	specfont 	= guiCreateFont("fonts/arialbold.ttf", 11)
	guiLabelSetColor(g_GUI.speclabel, 255 ,255 ,255)
	guiSetFont(g_GUI.speclabel, specfont)
...
function unloadAll()
	...
	if (guiGetVisible(mapinfobackground) == true) then
		destroyElement(mapinfobackground)
	else return end
...
------------------------------------------------------------------------------
-- showhud
-- NOTE: it's not completed, mapinfo is still shown.

isHUDShown = false;
function setHudShown()
		if (not isHUDShown) then
			isHUDShown = true;
			guiSetVisible(mapinfobackground, false)
			guiSetVisible(positonlabel, false)
			
			outputChatBox("#389583[RACE] #FFFFFFHUD has been hidden#389583. (#FFFFFFthis feature isn't stable#389583)", 255, 255, 255, true)
			hideGUIComponents('timeleftbg', 'timeleft', 'healthbar', 'speedbar', 'ranknum', 'ranksuffix', 'checkpoint', 'thetimepassed', 'clockimg', 'roundedimg', 'positonlabel', 'checkpointlabel')
		else
			isHUDShown = false;
			guiSetVisible(mapinfobackground, true)
			guiSetVisible(positonlabel, true)
			outputChatBox("#389583[RACE] #FFFFFFHUD has been shown#389583. (#FFFFFFthis feature isn't stable#389583)", 255, 255, 255, true)
			showGUIComponents('timeleftbg', 'timeleft', 'healthbar', 'speedbar', 'ranknum', 'ranksuffix', 'checkpoint', 'thetimepassed', 'clockimg', 'roundedimg', 'positonlabel', 'checkpointlabel')
		end 
	end
addCommandHandler("showracehud", setHudShown, false, false)
-------------------------------------------------------------------------------

local speedofont = dxCreateFont("fonts/arialbold.ttf", 11)
local yof = -899
local xof = 20
--screenWidth - 130, screenHeight - 80
...
function mph()
	...
	if car then
		if (getVehicleType(car) == "Plane") or (getVehicleType(car) == "Boat") or (getVehicleType(car) == "Helicopter") then
			...
			dxDrawText( speed, screenWidth - 130, screenHeight - 20, screenWidth - 130, screenHeight - 20, tocolor ( 255, 255, 255, 255 ), 1, speedofont, "right", "bottom", false, true, true)
		else
		 xx,yy,zz = getElementVelocity(car)
		 speednumber = getDistanceBetweenPoints3D(0,0,0,getElementVelocity(car))*100
		 speednumber = math.floor(speednumber)
...
function kmh()
	...
	if car then
		if (getVehicleType(car) == "Plane") or (getVehicleType(car) == "Boat") or (getVehicleType(car) == "Helicopter") then
		 ...
		 x,y = guiGetScreenSize()
		dxDrawText( speed, screenWidth - 45, screenHeight - 36, screenWidth - 45, screenHeight - 36, tocolor ( 255, 255, 255, 255 ), 1, speedofont, "right", "bottom", false, true, true)
	end	
	end
end
