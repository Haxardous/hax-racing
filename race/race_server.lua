	...
	if g_CurrentRaceMode.isPlayerFinished(player) then
		...
		if bPlayerJoined and g_CurrentRaceMode.running then
			...
			if g_GameOptions.joinspectating then
				...
				setPlayerStatus( player, nil, "joining..")
...
function updateNotReadyText()
	...
	if g_NotReadyDisplay then
		...
		if getTotalPlayerCount() > 1 then
			textItemSetText(g_NotReadyTextItems[1], ' ' )
		end
	end
end
...
addEventHandler("onPlayerFinish", root, function()
	...
	local oldpoints = getAccountData(acc, "race.finishes") or 0;
	local currentpoints = getAccountData(acc, "race.finishes") or 0;
    if (acc) and not isGuestAccount (acc) then
		  setAccountData (acc, "race.finishes", oldpoints+getPlayerCount()/1*4*2)
		  setElementData(source, "points", currentpoints+getPlayerCount()/1*4*2) 
		  outputChatBox("You've finished a map! points: ".. oldpoints+getPlayerCount()/1*4*2, source, 56 ,149 ,131)
    else
          outputChatBox("Please login to earn points.", source, 56 ,149 ,131)
    end
end
)

addCommandHandler("points", function(playerSource, commandName)
	acc = getPlayerAccount (playerSource)
	local currentpoints = getAccountData(acc, "race.finishes") or 0;
	if (acc) and not isGuestAccount (acc) then
	outputChatBox("Current Points: ( #FFFFFF".. currentpoints .." #389583)", playerSource, 56 ,149 ,131, true)
	end
end
)
...
addEventHandler("onResourceStop", root, function()
	exports.scoreboard:scoreboardRemoveColumn("points")
end)

addEventHandler("onPlayerLogin", root, function()
	acc = getPlayerAccount (source)
	local currentpoints = getAccountData(acc, "race.finishes") or 0;
	outputChatBox("Current Points: ( #FFFFFF".. currentpoints .." #389583)", source, 56 ,149 ,131, true)
	setElementData(source, "points", currentpoints) 
end)
...
function deleteResourceCommand(thePlayer, commandName, resourceName)
	...
	if resourceName then
		...
		outputChatBox("#389583[RACE] #FFFFFFWrite a resource name#389583.", thePlayer, 255, 255, 255, true)
	end
	else
		outputChatBox("#389583[RACE] #FFFFFFyou don't have permission#389583.", thePlayer, 255, 255, 255, true)
	end
end
...
addCommandHandler("great", function(player)
	outputChatBox("#389583[RATE] ".. getPlayerName(player).." #FFFFFFsays this map is #00FF00great#FFFFFF.", root, 255, 255, 255, true)
end)

addCommandHandler("gay", function(player)
	outputChatBox("#389583[RATE] ".. getPlayerName(player).." #FFFFFFsays this map is #ff0000g#ff7f00g#ffff00a#00ff00a#00ffffy#0000ffy#FFFFFF.", root, 255, 255, 255, true)
end)

addCommandHandler("good", function(player)
	outputChatBox("#389583[RATE] ".. getPlayerName(player).." #FFFFFFsays this map is #32CD32good#FFFFFF.", root, 255, 255, 255, true)
end)

addCommandHandler("bad", function(player)
	outputChatBox("#389583[RATE] ".. getPlayerName(player).." #FFFFFFsays this map is #FF0000bad#FFFFFF.", root, 255, 255, 255, true)
end)

addCommandHandler("shit", function(player)
	outputChatBox("#389583[RATE] ".. getPlayerName(player).." #FFFFFFsays this map is #8B4513shit#FFFFFF.", root, 255, 255, 255, true)
end)

addCommandHandler("meh", function(player)
	outputChatBox("#389583[RATE] ".. getPlayerName(player).." #FFFFFFsays this map is #FFA500meh#FFFFFF.", root, 255, 255, 255, true)
end)

addCommandHandler("cool", function(player)
	outputChatBox("#389583[RATE] ".. getPlayerName(player).." #FFFFFFsays this map is #00BFFFcool#FFFFFF.", root, 255, 255, 255, true)
end)

addCommandHandler("neutral", function(player)
	outputChatBox("#389583[RATE] ".. getPlayerName(player).." #FFFFFFsays this map is #FFFF00neutral#FFFFFF.", root, 255, 255, 255, true)
end)

addCommandHandler("badcp", function(player)
	outputChatBox("#389583[RATE] ".. getPlayerName(player).." #FFFFFFsays this map has #FF0000bad checkpoints#FFFFFF.", root, 255, 255, 255, true)
end)

addCommandHandler("bt", function(player)
	outputChatBox("#389583[RATE] ".. getPlayerName(player).." #FFFFFFsays this map has #FF0000bad turns#FFFFFF.", root, 255, 255, 255, true)
end)

addCommandHandler("bl", function(player)
	outputChatBox("#389583[RATE] ".. getPlayerName(player).." #FFFFFFsays this map has #FF0000bad lines#FFFFFF.", root, 255, 255, 255, true)
end)

-----------------------------------------------------------------------------------

function playerChat(message, messageType)
	thepointsrank = getElementData(source, "points") or 0;
	setpointsrank = nil

	if thepointsrank < 20 then
		setpointsrank = "Newbie"
	 elseif thepointsrank < 100 then
		setpointsrank = "#C0C0C0Racer"
	 elseif thepointsrank < 300 then
		setpointsrank = "#C0C0C0Racer I"
	 elseif thepointsrank < 500 then
		setpointsrank = "#C0C0C0Racer II"
	 elseif thepointsrank < 750 then
		setpointsrank = "#C0C0C0Racer III"
	 elseif thepointsrank < 1000 then
		setpointsrank = "#C0C0C0Racer IV"
	 elseif thepointsrank < 1500 then
		setpointsrank = "#FFD700Racer V"
	 elseif thepointsrank < 2000 then
		setpointsrank = "#FFD700Racer VI"
	 elseif thepointsrank < 2500 then
		setpointsrank = "#FFD700Racer VII"
	 elseif thepointsrank < 3000 then
		setpointsrank = "#FFD700Racer VII"
	 elseif thepointsrank < 3500 then
		setpointsrank = "#FFD700Racer VIII"
	 elseif thepointsrank < 4000 then
		setpointsrank = "#7B68EERacer IX"
	 elseif thepointsrank < 5000 then
		setpointsrank = "#7B68EERacer X"
	 elseif thepointsrank < 6500 then
		setpointsrank = "#7B68EERacer XXI"
	 elseif thepointsrank < 8000 then
		setpointsrank = "#7B68EERacer XXII"
	 else
		setpointsrank = "N/A"
	 end
	 accName = getAccountName ( getPlayerAccount ( source ) ) 
	 if isObjectInACLGroup ("user."..accName, aclGetGroup ( "A" ) ) then
	 if messageType == 0 then
			 cancelEvent()
			 outputChatBox("#389583A #FFFFFF- #389583".. setpointsrank .."#FFFFFF "..getPlayerNametagText(source)..": #FFFFFF"..message, root, 255, 255 ,255, true )
			 outputServerLog("CHAT: "..getPlayerName(source)..": "..message)
	 end
		 else
	--[[	elseif thepointsrank > 100 then thepointsrank = "New"
	elseif thepointsrank > 200 then thepointsrank = "Racer"
	elseif thepointsrank > 300 then thepointsrank = "Racer I"
	elseif thepointsrank > 400 then thepointsrank = "Racer II"
	elseif thepointsrank > 500 then thepointsrank = "Racer III"
	elseif thepointsrank > 600 then thepointsrank = "Racer IV"
	elseif thepointsrank > 600 then thepointsrank = "Racer VI"
	elseif thepointsrank > 600 then thepointsrank = "Racer VII"
	elseif thepointsrank > 600 then thepointsrank = "Racer VIII"
	elseif thepointsrank > 600 then thepointsrank = "Racer IX"
	elseif thepointsrank > 600 then thepointsrank = "Racer X"]]
	if messageType == 0 then
            cancelEvent()
			outputChatBox("#389583".. setpointsrank .. "#FFFFFF "..getPlayerNametagText(source)..": #FFFFFF"..message, root, 255, 255 ,255, true )
			outputServerLog("CHAT: "..getPlayerName(source)..": "..message)
	end
	end
end
...
setTimer(
    function(player)
        for _, player in ipairs(getElementsByType("player")) do 
			if (getPlayerIdleTime(player) > 45000) then 
				if (getElementData(player, "state") == "AFK") then
			else
				setPlayerStatus( player, "AFK", "" )
				end
			else
				outputChatBox("* "..getPlayerNametagText(player).." #FFFFFFis now AFK.", root, 255, 255, 255, true)
			end
        end
    end,
45000, 0)
...
addCommandHandler("afk", function(player)
	if (getElementData(player, "state") == "AFK") then
		setPlayerStatus(player, "Available", "")
		outputChatBox("* "..getPlayerNametagText(player).." #FFFFFFis back.", root, 255, 255, 255, true)
else
	setPlayerStatus( player, "AFK", "" )
	outputChatBox("* "..getPlayerNametagText(player).." #FFFFFFis now AFK.", root, 255, 255, 255, true)
	end
...
addEventHandler("onPlayerChat", getRootElement(), function()
	if (getElementData(source, "state") == "AFK") then
		setPlayerStatus(source, "Available", "")
	outputChatBox("* "..getPlayerNametagText(source).." #FFFFFFis back.", root, 255, 255, 255, true)
	end
end
)
