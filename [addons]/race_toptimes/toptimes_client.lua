...
function CToptimes:openWindow ()
	...
	arialfont = guiCreateFont("fonts/arialbold.ttf", 11)
	...
	guiSetAlpha(self.gui['windowbg'], 255)
	guiSetVisible( self.gui['windowbg'], false )

	self.gui['hash'] = guiCreateStaticImage(20, 37, 12, 15, 'img/hash.png', false, self.gui['windowbg'])
	guiSetProperty(self.gui['hash'], "AlwaysOnTop", "true")

	self.gui['user'] = guiCreateStaticImage(85, 37, 15, 15, 'img/user.png', false, self.gui['windowbg'])
	guiSetProperty(self.gui['user'], "AlwaysOnTop", "true")

	self.gui['time'] = guiCreateStaticImage(220, 37, 12, 15, 'img/time.png', false, self.gui['windowbg'])
	guiSetProperty(self.gui['time'], "AlwaysOnTop", "true")
	...
	-- windowbg as parent: 
		...
		-- container as parent:
			...
			--guiSetAlpha(self.gui['bar'], 255)

			self.gui['title'] = guiCreateLabel(0, 6, sizeX, 15, 'Top Times - ', false, self.gui['container'] )
			guiLabelSetHorizontalAlign ( self.gui['title'], 'center' )
			guiSetFont(self.gui['title'], arialfont)
			guiLabelSetColor ( self.gui['title'], 220, 220, 225 )

			--self.gui['header'] = guiCreateLabel(19, 37, sizeX-30, 15, '        Name                          Time', false, self.gui['container'] )
			--guiSetFont(self.gui['header'], arialfont)
			--guiLabelSetColor ( self.gui['header'], 192, 192, 192 )

			self.gui['headerul'] = guiCreateLabel(0, 12, sizeX, 15, string.rep('_', 50), false, self.gui['container'] )
			guiLabelSetHorizontalAlign ( self.gui['headerul'], 'center' )
			guiLabelSetColor ( self.gui['headerul'], 192, 192, 192 )

			--self.gui['headerultwo'] = guiCreateLabel(0, 45, sizeX, 15, string.rep('_', 50), false, self.gui['container'] )
			--guiLabelSetHorizontalAlign ( self.gui['headerultwo'], 'center' )
			--guiLabelSetColor ( self.gui['headerultwo'], 192, 192, 192 )
			...
			-- paneLoading as parent:
				...
				guiSetFont(self.gui['busy'], arialfont)
				guiSetFont(self.gui['busy2'], arialfont)
...
function CToptimes:updateLabelCount(numLines)
	...
	while #t < numLines do
		...
		local label = guiCreateLabel(x, 65+22*y, sizeX-x-10, 15, '', false, parentGui )
		guiSetFont(label, arialfont)
...
function CToptimes:doOnServerSentToptimes( data, serverRevision, playerPosition )
	...
	for i=1,numLines do
		...
		local line = string.format( '%d.  %s   %s', i, data[i].playerName, timeText  )
		guiSetText ( self.gui['listTimes'][i], line )

		if i == playerPosition then
			guiLabelSetColor ( self.gui['listTimes'][i], 100, 220, 100 )
		else
			guiLabelSetColor ( self.gui['listTimes'][i], 230, 230, 230 )
		end
...
function CToptimes:doOnClientRender()
	...
	-- Apply
	guiSetAlpha( self.gui['windowbg'], self.currentFade * 0.95 )
