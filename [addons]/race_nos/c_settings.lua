...
function consoleSetNosFiringStyle(commandName, nosControl)
	...
	if nosControl ~= "hybrid" and nosControl ~= "nfs" and nosControl ~= "normal" then
		if commandName == "nos" then
			outputChatBox("#389583[NOS] Syntax: #FFFFFF/nos [normal|nfs|hybrid]#389583.", 255, 255, 255, true)
		else
			outputChatBox("#389583[NOS] Syntax: #FFFFFF/noscontrol [normal|nfs|hybrid]#389583.", 255, 255, 255, true)
		end
		return
	end
	if g_Settings.ControlStyle ~= nosControl then
		...
		if saveSettingsToFile() then
			outputChatBox("#389583[NOS] #FFFFFFYour NOS control style has been set to '#389583" .. tostring(settingName) .. "#FFFFFF'.", 255, 255, 255, true)
		end
	end
end
...
function consoleSetGaugeVisibility(commandName, visibility)
	if visibility ~= "on" and visibility ~= "off" then
		outputChatBox("#389583[NOS] Syntax: #FFFFFF/nosgauge [on|off]##389583.", 255, 255, 255, true)
		return
	end
...
function consoleSetNosSustainBehavior(commandName, arg)
	if arg ~= "on" and arg ~= "off" then
		outputChatBox("#389583[NOS] #FFFFFFSyntax: /nossustain [on|off]#389583.", 255, 255, 255, true)
		return
	end
	...
	if saveSettingsToFile() then
		outputChatBox("#389583[NOS] #FFFFFFNOS sustain behavior is now ##389583" .. arg .. ".", 255 ,255 ,255, true)
	end
...
function maybeDisplaySpecialMessage()
	...
	if counter==1 then
		--outputChatBox ( "** Note NOS has changed a little bit **" , 255 ,255 ,255, true)
	end
	if counter==2 then
		--outputChatBox ( "** Just incase you missed this message last time **" , 255 ,255 ,255, true)
	end
	if counter==6 then
	...
	if counter==1 or counter==2 or counter==6 then
		outputChatBox ( "#389583[NOS] #FFFFFFIf you want normal nos type: /nos normal#389583." , 255 ,255 ,255, true)
		outputChatBox ( "#389583[NOS] #FFFFFFIf you want NFS style nos type: /nos nfs#389583." , 255 ,255 ,255, true)
	end
